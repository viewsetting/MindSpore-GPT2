...
from src.utils.tensor_manipulations import modify_inputMask
>>>>>>> Stashed changes
...
def do_train(dataset=None, network=None, load_checkpoint_path="", save_checkpoint_path="", epoch_num=1):
    ...
    ckpoint_cb = ModelCheckpoint(prefix="gpt2_summarization_noBOS_",
...
def do_eval(dataset=None, network=None, metric=None, load_checkpoint_path="",TL_DR=True):
    """
    ...
    if metric.lower() == "rouge":
        ...
        for netName in param_dict:
            print(netName)
            if 'gpt2_embedding_lookup.embedding_table' in netName and 'lamb' not in netName:
                embedding_table_name = netName
            reorganized_param_dict['gpt2.'+netName] = param_dict[netName]

        if embedding_table_name != "":
            reorganized_param_dict['lm_head.weight'] = param_dict[embedding_table_name]
        else:
            raise ValueError('There is no weight of gpt2_embedding_lookup.embedding_table in {} file.'.format(load_checkpoint_path))
        ...
        sample = Sample(model,tokenizer=tokenizer,model_config=gpt2_net_cfg,topk_num = 0,topp_prob=0.9,min_tokens_to_keep=1,demo_mode=False)
        ...
        for data in dataset.create_dict_iterator():
            ...
            #print('top_p:',sample.topp_prob,' top_k:',sample.topk_num) 
            hypo,ref = sample.generate_for_CNN_DAILYMAIL(input_ids,generate_length=100,select_sentence=3,TL_DR=TL_DR)
            # article,ref = sample._extract_string_from_tensor(input_ids,mode='pair')
            # print(article)
            # hypo,_ = sample.generate(article,generate_length=50)
            # hypo = [hypo]
            # ref = [ref]

            for idx in range(gpt2_net_cfg.batch_size):
                if hypo[idx] == '':
                    hypo[idx] = '<empty>'
                if ref[idx] == '':
                    ref[idx] = '<empty>'
...
def run_summarization():
    ...
    parser.add_argument("--load_finetune_ckpt_path", type=str, default="/datasets/pretrained_weights/ms_model_small.ckpt",
                        help="Load the checkpoint file path.")
    parser.add_argument("--train_data_file_path", type=str, default="/datasets/cnn_dailymail/cnn_dailymail-train_noBOS-mindrecord",
                        help="Data path, it is better to use absolute path")
    parser.add_argument("--eval_data_file_path", type=str, default="/datasets/cnn_dailymail/cnn_dailymail-test-mindrecord",
                        help="Data path, it is better to use absolute path")

    parser.add_argument("--TL_DR", type=bool, default=True,
                        help="for evaluation, True if there is a TL,DR; hint for generation")
    ...
    tl_dr = args_opt.TL_DR
    train_data_file_path = args_opt.train_data_file_path
    eval_data_file_path = args_opt.eval_data_file_path
    ...
    if args_opt.do_eval.lower() == "true":
        ...
        eval_dataset = create_cnn_dailymail_dataset(
            dataset_path=eval_data_file_path)
        do_eval(eval_dataset, GPT2SummarizationModel, metric, load_finetune_ckpt_path,TL_DR=tl_dr)
